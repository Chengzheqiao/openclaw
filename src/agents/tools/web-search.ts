...
const SEARCH_PROVIDERS = ["serper", "brave", "perplexity", "grok", "gemini", "kimi"] as const;
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "serper") {
    return {
      ...
      message: `web_search needs a Serper API key. Run \`${formatCliCommand("openclaw configure --section web")}\` to store it, or set SERPER_API_KEY in the Gateway environment.`,
...
export function createWebSearchTool(options?: {
  ...
  const description =
    provider === "serper"
      ? "Search the web using Serper (Google Search). Returns titles, URLs, and snippets for fast research. Supports region-specific and localized search via country and language parameters."
      : provider === "perplexity"
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (country && provider !== "serper" && provider !== "brave" && provider !== "perplexity") {
        return jsonResult({
          error: "unsupported_country",
          message: `country filtering is not supported by the ${provider} provider. Only Serper, Brave, and Perplexity support country filtering.`,
          docs: "https://docs.openclaw.ai/tools/web",
        });
      }
      const language = readStringParam(params, "language");
      if (language && provider !== "serper" && provider !== "brave" && provider !== "perplexity") {
        return jsonResult({
          error: "unsupported_language",
          message: `language filtering is not supported by the ${provider} provider. Only Serper, Brave, and Perplexity support language filtering.`,
          docs: "https://docs.openclaw.ai/tools/web",
        });
      }
